...
# Use global values for consistent vertical spacing
global_y_max = df_all["signal_sum"].max()
y_base = global_y_max * 1.05
bar_height = global_y_max * 0.05
...
for i, (cell_line, df_sub) in enumerate(df_all.groupby("cell_line")):
    ...
    # Add p-value bars (shared y_base and bar_height across both panels)
    ...
    for j, treatment in enumerate(treatment_order[1:], start=1):
        ...
        # Format p-value for figure (3 significant digits)
        formatted_p = f"p = {pval:.3g}"

        x1, x2 = 0, j
        y = y_base + bar_height * 1.2 * (j - 1)

        ax.plot([x1, x1, x2, x2], [y, y + bar_height * 0.2, y + bar_height * 0.2, y], lw=1.3, c='black')
        ax.text((x1 + x2) / 2, y + bar_height * 0.35, formatted_p,  # Adjust text slightly higher
...
plt.suptitle("Fluorescence Signal in Zebrafish (NOMO-1 vs MOLM-13)", fontsize=15, y=1.05)
plt.tight_layout()
